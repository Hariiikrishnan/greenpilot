...
// Typed Meta API failure (HTTP status travels with the error).
...
async function getMediaInfo(mediaId, accessToken) {
  ...
  if (!res.ok) {
    ...
    throw new MetaApiError(`Meta getMediaInfo ${res.status}: ${body.slice(0, 200)}`, {
      status: res.status,
    });
  }
  return res.json();
...
async function downloadMediaBinary(url, accessToken) {
  ...
  const res = await fetch(url, {
    ...
    headers: {
      ...
      'User-Agent': 'GreenPilot/1.0',
    },
  });
  if (!res.ok) {
    const body = await res.text().catch(() => '');
    throw new MetaApiError(`Meta downloadMediaBinary ${res.status}: ${body.slice(0, 200)}`, {
      status: res.status,
    });
  }
