...
// Typed Meta API failure (HTTP status + provider body travel with the error
// so classifiers and logs can use them without re-fetching).
class MetaApiError extends Error {
  /**
   * @param {string} message
   * @param {{status?: number, metaError?: any, body?: any}} [details]
   */
  constructor(message, { status, metaError, body } = {}) {
    super(message);
...
async function postJson(endpoint, accessToken, body) {
  ...
  if (!res.ok) {
    ...
    throw new MetaApiError(`Meta ${res.status}: ${errMsg}`, {
      status: res.status, metaError: parsed?.error || null, body: parsed,
    });
  }
  return parsed;
}
...
async function uploadMedia({ accessToken, phoneNumberId, buffer, mimeType, filename }) {
  ...
  try { parsed = JSON.parse(text); } catch { /* non-JSON body — handled below */ }
  if (!res.ok) {
    throw new MetaApiError(`Meta upload ${res.status}: ${parsed?.error?.message || text.slice(0, 200)}`, {
      status: res.status,
    });
  }
  return parsed; // { id }
