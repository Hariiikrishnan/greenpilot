...
function startWorker() {
  ...
  worker = new Worker(QUEUE_NAME, async (job) => {
    ...
    const result = await downloadOne(messageId, organizationId || null);
    if (!result.ok) {
 ...
 * enqueues (e.g. webhook retries) collapse to a single job. The organizationId
 * travels with the job so the worker confines its row access to that tenant.
 */
async function enqueueMediaDownload(messageId, organizationId = null) {
  if (!messageId) return;
  try {
    await mediaQueue.add('download', { messageId, organizationId }, {
    ...
    downloadOne(messageId, organizationId).catch(e => console.error('[mediaQueue] inline fallback error:', e.message));
  }
}
