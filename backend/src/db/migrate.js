...
function migrationsDir() {
  // Resolve across layouts: MIGRATIONS_DIR override, the repo (<repo>/db/migrations),
  // and the container image (/app/src/db -> /app/db/migrations).
  const candidates = [
    process.env.MIGRATIONS_DIR,
    path.join(__dirname, '../../../db/migrations'),
    path.join(__dirname, '../../db/migrations'),
  ].filter(Boolean);
  return candidates.find(d => fs.existsSync(d) && fs.existsSync(path.join(d, '000_base_users.sql')))
