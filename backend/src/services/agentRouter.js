...
async function routeIfActive(record) {
  ...
  const { rows } = await pool.query(
    `SELECT a.id, a.wa_account_id, a.trigger_mode, a.trigger_keyword,
            ...
            w.organization_id AS account_org_id
  ...
  // Step 10 tenant gate: when the inbound record already carries an org (v1
  // webhook path), the routed agent's account must belong to it.
  if (record.organizationId && agent.account_org_id &&
      String(record.organizationId) !== String(agent.account_org_id)) {
    return null;
  }
  ...
  if (await isConversationPaused(record.wa_number, record.contact_number, record.organizationId || agent.account_org_id || null)) {
  ...
  if (agent.handoff_enabled && hasText) {
    ...
    if (matchesAnyHandoffKeyword(record.message_body, agent.handoff_keywords)) {
      await performHandoff({
        ...
        organizationId: record.organizationId || agent.account_org_id || null,
  ...
  await enqueueAgentRun({
    ...
    organizationId: agent.account_org_id || record.organizationId || null,
  });
