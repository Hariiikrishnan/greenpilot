...
async function reconcileMessageStatuses({ windowDays = 2 } = {}) {
  const { rows } = await pool.query(`
    ...
    UPDATE coexistence.chat_history ch
     ...
     RETURNING ch.wa_number, ch.contact_number, ch.message_id, ch.status, ch.organization_id
  `, [String(windowDays)]);
  ...
  for (const r of rows) {
    ...
    // Phase 7: tenant-scoped socket emit. The row's stamped org is the ONLY
    // source — rows without an org stay SSE-only (never guessed).
    if (r.organization_id) {
      try {
        require('../realtime/emitter').emitMessageStatus(r.organization_id, {
          waNumber: r.wa_number,
          contactNumber: r.contact_number,
          messageId: r.message_id,
          status: r.status,
        ...
        console.error('[status-reconcile] realtime emit failed:', err.message);
      }
    }
  }
  return rows.length;
