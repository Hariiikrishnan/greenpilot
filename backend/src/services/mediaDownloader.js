...
async function transcodeToMp3(srcPath) {
  ...
  try {
    ...
    try { fs.unlinkSync(outPath); } catch { /* best-effort partial-output cleanup */ }
    return null;
  }
}

async function markStatus(messageId, status, fields = {}, organizationId = null) {
  ...
  // Phase 13: when the caller knows the tenant, confine the update to it
  // (dual-read). Unscoped callers keep legacy behavior.
  const orgScope = organizationId
    ? ` AND (organization_id = $${i + 1} OR organization_id IS NULL)`
    : ``;
  if (organizationId) params.push(organizationId);
  await pool.query(
    `UPDATE coexistence.chat_history SET ${sets.join(', ')} WHERE message_id = $${i}${orgScope}`,
    params
  );
}
 ...
 * When organizationId is supplied (webhook/queue path), the row lookup and all
 * status writes are confined to that org scope (dual-read) so a forged job
 * can never pull or stamp another tenant's media.
 * Returns { ok: boolean, path?: string, error?: string }.
 */
async function downloadOne(messageId, organizationId = null) {
  const lookupParams = organizationId ? [messageId, organizationId] : [messageId];
  const { rows } = await pool.query(
    `SELECT message_id, wa_number, message_type, media_url, media_mime_type, media_status, media_storage_path, timestamp
      ...
      WHERE message_id = $1${organizationId ? ` AND (organization_id = $2 OR organization_id IS NULL)` : ``}`,
    lookupParams
  );
  const row = rows[0];
  ...
  try {
    await markStatus(messageId, 'downloading', { media_error: null }, organizationId);
    ...
    if (TRANSCODE_AUDIO && (row.message_type === 'audio' || row.message_type === 'voice')
      ...
      if (transcoded) {
        try { fs.unlinkSync(finalPath); } catch { /* best-effort source cleanup after transcode */ }
    ...
    await markStatus(messageId, status, { media_error: (err.message || String(err)).slice(0, 500) }, organizationId).catch(() => {});
...
async function markPending(messageId) {
  try { await markStatus(messageId, 'pending'); } catch { /* best-effort status hint — download proceeds regardless */ }
}
