...
//
// Phase 9 tenancy: rows are global-by-default (NULL organization_id, the
// documented Phase 6 exception for shared provider fallbacks) with optional
// org stamping. List/get dual-read (global + own org); creates stamp the
// acting org; org-stamped rows mutate only through their org, global rows
// through any org admin (shared credential stewardship).
...
// Resolve a model row visible to the acting org (global or own), else null.
async function visibleModel(id, orgId) {
  const { rows } = await pool.query(
    `SELECT * FROM coexistence.ai_models
      WHERE id = $1
...
router.get('/ai-models/:id', authed, async (req, res) => {
  try {
    const row = await visibleModel(req.params.id, req.org?.id || null);
    if (!row) return res.status(404).json({ error: 'Not found' });
    const reveal = req.query.reveal === '1' && req.user?.role === 'admin';
    res.json(shape(row, { reveal }));
  } catch (err) {
...
router.post('/ai-models', adminOnly, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO coexistence.ai_models (provider, label, api_key_encrypted, available_models, organization_id)
       VALUES ($1, $2, $3, '[]'::jsonb, $4)
       RETURNING *`,
      [provider, b.label?.trim() || null, encrypt(apiKey), req.org?.id || null],
    );
...
router.put('/ai-models/:id', adminOnly, async (req, res) => {
  try {
    const org = req.org?.id || null;
    const current = await visibleModel(req.params.id, org);
    if (!current) return res.status(404).json({ error: 'Not found' });
    // Org-stamped rows mutate only through their org; global rows stay shared.
    if (current.organization_id && String(current.organization_id) !== String(org)) {
