...
async function transcodeAudioForMeta(srcBuffer, srcMime) {
  ...
  try {
    ...
    try { fs.unlinkSync(tmpIn); } catch { /* best-effort temp cleanup */ }
    try { fs.unlinkSync(tmpOut); } catch { /* best-effort temp cleanup */ }
  }
}
...
router.get('/numbers', async (req, res) => {
  try {
    // Phase 13 tenancy: numbers, counts, and unreads are scoped to the acting
    // org (own rows + legacy NULL rows; legacy callers see legacy rows only).
    // The org id is bound once ($n) and reused by every predicate below.
    ...
    const orgPred = (alias) => orgPh
      ? ` AND (${alias}.organization_id = ${orgPh} OR ${alias}.organization_id IS NULL)`
    ...
    if (!isAdmin(req.user)) {
      ...
      extraFilter = `AND wa_number IN (
        SELECT DISTINCT wa_number FROM coexistence.contacts WHERE assigned_user_id = $${params.length}${orgPred('contacts')}
      )`;
    }
    const { rows } = await pool.query(`
      ...
      WHERE timestamp >= NOW() - ${DEFAULT_DATA_WINDOW} ${extraFilter}${orgPred('chat_history')}
        ...
        AND regexp_replace(wa_number, '[^0-9]', '', 'g') IN (
          ...
          WHERE display_phone_number IS NOT NULL
            ${orgPh ? `AND (organization_id = ${orgPh} OR organization_id IS NULL)` : `AND organization_id IS NULL`}
        )
      GROUP BY wa_number
    ...
    const unreadParams = orgPh ? [orgId] : [];
    const unreadOrgPh = orgPh ? '$1' : null;
    let unreadJoin = '';
    let unreadAssign = '';
    if (!isAdmin(req.user)) {
      ...
      unreadJoin = `JOIN coexistence.contacts c
        ...
        ${unreadOrgPh ? `AND (c.organization_id = ${unreadOrgPh} OR c.organization_id IS NULL)` : `AND c.organization_id IS NULL`}`;
    ...
    const unreadRes = await pool.query(`
      ...
      WHERE ch.direction = 'incoming'
        ...
        ${unreadOrgPh ? `AND (ch.organization_id = ${unreadOrgPh} OR ch.organization_id IS NULL)` : `AND ch.organization_id IS NULL`}
        ${unreadAssign}
    ...
    if (waNumbers.length > 0) {
      ...
      const [nameRes, teamRes] = await Promise.all([
        ...
        // display name. Org-scoped so one tenant never resolves another's name.
        pool.query(
          `SELECT wa_number, COALESCE(name, profile_name) AS name
             ...
             WHERE contact_number = wa_number AND wa_number = ANY($1::text[])
               ${orgPh ? `AND (organization_id = $2 OR organization_id IS NULL)` : `AND organization_id IS NULL`}`,
          orgPh ? [waNumbers, orgId] : [waNumbers]
        ),
...
router.get('/contacts', async (req, res) => {
  try {
    ...
    // Phase 11 tenancy: history + contact rows strictly inside the acting org
    // (legacy callers see legacy rows). Result stays an array, bounded.
    const orgId = req.org?.id || null;
    const orgFilter = orgId ? 'AND ch.organization_id = $ORG' : 'AND ch.organization_id IS NULL';
    const orgSub = orgId
      ? 'AND ch2.organization_id = ch.organization_id'
      : 'AND ch2.organization_id IS NULL';
    const orgJoin = orgId
      ? 'AND c.organization_id = ch.organization_id'
      : 'AND c.organization_id IS NULL';
    const orgParams = orgId ? [orgId] : [];
    const orgPh = orgId ? `$${params.length + 1}` : null;
    const { rows } = await pool.query(`
      SELECT
        ch.contact_number,
      ...
      LEFT JOIN coexistence.conversation_reads cr
        ON cr.wa_number = ch.wa_number AND cr.contact_number = ch.contact_number
      WHERE ch.wa_number = $1 ${timeFilter} ${assignFilter} ${orgId ? orgFilter.replace('$ORG', orgPh) : orgFilter}
  ...
  try {
    ...
    if (!admin) {
      ...
      if (assignedUserId === null || assignedUserId === '') {
        ...
        assignVal = parseInt(assignedUserId, 10);
        // Phase 11: admins may only assign active members of the acting org.
        if (req.org?.id) {
          const { rows: mRows } = await pool.query(
            `SELECT u.id FROM coexistence.forgecrm_users u
              JOIN coexistence.organization_members m ON m.user_id = u.id AND m.organization_id = $2
             WHERE u.id = $1 AND u.is_active = TRUE`,
            [assignVal, req.org.id]
          );
          if (!mRows[0]) {
    ...
    // Phase 11 tenancy: new rows are stamped with the acting org; existing
    // rows keep their org, and an org caller can never write a row owned by
    // another org (the upsert below targets the scoped row — a foreign row
    // simply isn't matched, and the INSERT would conflict; the pre-check
    // turns that into an explicit 403 instead of a 500).
    const saveOrgId = req.org?.id || null;
    if (saveOrgId) {
      ...
      if (scopeRows[0] && scopeRows[0].organization_id &&
          String(scopeRows[0].organization_id) !== String(saveOrgId)) {
        return res.status(403).json({ error: 'Contact belongs to another organization' });
      }
    }

    await pool.query(`
      INSERT INTO coexistence.contacts
        (wa_number, contact_number, organization_id, name, tags, custom_fields, assigned_user_id, updated_at)
      VALUES ($1, $2, $8, $3, $4, COALESCE($5::jsonb, '{}'::jsonb), $6, NOW())
      ...
      DO UPDATE SET
        ...
        updated_at = NOW()
    `, [waNumber, contactNumber, cleanName, JSON.stringify(tags || []), cf, assignVal, setAssign, saveOrgId]);
...
router.post('/contacts/import', sheetUpload.single('file'), async (req, res) => {
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Phase 11: imported rows are stamped with the acting org. Cross-org
      // pairs resolve to the existing row untouched (fail closed below).
      const impOrgId = req.org?.id || null;
      if (impOrgId) {
        const { rows: scopeRows } = await pool.query(
          `SELECT organization_id FROM coexistence.contacts WHERE wa_number = $1 AND contact_number = $2`,
          [waNumber, phone]
        ...
        if (scopeRows[0] && scopeRows[0].organization_id &&
            String(scopeRows[0].organization_id) !== String(impOrgId)) {
          skipped.push({ row: rowNum, reason: 'Contact belongs to another organization' });
          continue;
        }
      }
      let result;
      if (admin) {
        result = await pool.query(`
          INSERT INTO coexistence.contacts (wa_number, contact_number, organization_id, name, tags, custom_fields, updated_at)
          VALUES ($1, $2, $4, $3, '[]'::jsonb, '{}'::jsonb, NOW())
          ON CONFLICT (wa_number, contact_number)
          DO UPDATE SET name = COALESCE(EXCLUDED.name, coexistence.contacts.name), updated_at = NOW()
          RETURNING (xmax = 0) AS inserted
        `, [waNumber, phone, name, impOrgId]);
      } else {
        ...
        result = await pool.query(`
          INSERT INTO coexistence.contacts (wa_number, contact_number, organization_id, name, tags, custom_fields, assigned_user_id, updated_at)
          VALUES ($1, $2, $5, $3, '[]'::jsonb, '{}'::jsonb, $4, NOW())
          ON CONFLICT (wa_number, contact_number)
          DO UPDATE SET name = COALESCE(EXCLUDED.name, coexistence.contacts.name),
                        ...
                        updated_at = NOW()
          RETURNING (xmax = 0) AS inserted
        `, [waNumber, phone, name, req.user.id, impOrgId]);
      }
...
router.post('/contacts/change-number', async (req, res) => {
  ...
  // Phase 11 tenancy: the source row must live in the acting org (strict),
  // and every migrated table is scoped to that org so a shared global number
  // pair can never drag another tenant's rows along.
  const cnOrgId = req.org?.id || null;
  // Parameterized scope fragment: $4 binds the org (or IS NULL branch).
  const cnScope = (alias) => cnOrgId
    ? `AND ${alias}.organization_id = $4`
    : `AND ${alias}.organization_id IS NULL`;
  const cnParams = (extra = []) => (cnOrgId ? [waNumber, oldNumber, newNumber, cnOrgId, ...extra] : [waNumber, oldNumber, newNumber, ...extra]);
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    const src = cnOrgId
      ? await client.query(
          `SELECT organization_id FROM coexistence.contacts
            WHERE wa_number = $1 AND contact_number = $2 AND organization_id = $3`,
          [waNumber, oldNumber, cnOrgId]
        )
      : await client.query(
          `SELECT organization_id FROM coexistence.contacts
    ...
    // Migrate every table keyed on (wa_number, contact_number), org-scoped.
    const migrate = (table, waCol) => client.query(
      `UPDATE coexistence.${table} SET contact_number = $3, updated_at = NOW()
        WHERE ${waCol} = $1 AND contact_number = $2 ${cnScope(`coexistence.${table}`)}`,
      cnParams()
    );
    const migrateNoTs = (table, waCol) => client.query(
      `UPDATE coexistence.${table} SET contact_number = $3
        WHERE ${waCol} = $1 AND contact_number = $2 ${cnScope(`coexistence.${table}`)}`,
      cnParams()
    );
    await migrate('contacts', 'wa_number');
    await migrateNoTs('chat_history', 'wa_number');
    await migrateNoTs('conversation_reads', 'wa_number');
    await migrateNoTs('message_reactions', 'wa_number');
    await migrateNoTs('automation_executions', 'wa_number');
    await migrateNoTs('deals', 'contact_wa_number');
    await migrateNoTs('lead_activities', 'wa_number');
...
router.delete('/contact', async (req, res) => {
  try {
    ...
    // Phase 11: strictly inside the acting org.
    if (!(await assertContactAccess(req, res, waNumber, contactNumber))) return;
    const delOrgId = req.org?.id || null;
    const { rowCount } = delOrgId
      ? await pool.query(
          `DELETE FROM coexistence.contacts WHERE wa_number = $1 AND contact_number = $2 AND organization_id = $3`,
          [waNumber, contactNumber, delOrgId]
        )
      : await pool.query(
          `DELETE FROM coexistence.contacts WHERE wa_number = $1 AND contact_number = $2 AND organization_id IS NULL`,
          [waNumber, contactNumber]
        );
...
router.get('/saved-contacts', async (req, res) => {
  try {
    ...
    // Phase 11: org scope + server cap (array shape preserved for the UI).
    const scOrgId = req.org?.id || null;
    const scClause = scOrgId
      ? `AND c.organization_id = $${params.length + 1}`
      : `AND c.organization_id IS NULL`;
    if (scOrgId) params.push(scOrgId);
    const { rows } = await pool.query(`
      ...
      LEFT JOIN coexistence.forgecrm_users u ON u.id = c.assigned_user_id
      WHERE c.wa_number = $1 AND COALESCE(c.name, c.profile_name) IS NOT NULL AND COALESCE(c.name, c.profile_name) <> '' ${assignFilter} ${scClause}
  ...
  try {
    ...
    const { rows } = coOrgId
      ? await pool.query(`
        ...
        FROM coexistence.contacts c
        LEFT JOIN coexistence.forgecrm_users u ON u.id = c.assigned_user_id
        WHERE c.wa_number = $1 AND c.contact_number = $2 AND c.organization_id = $3
        LIMIT 1
      `, [waNumber, contactNumber, coOrgId])
      : await pool.query(`
        SELECT c.contact_number, COALESCE(c.name, c.profile_name) AS name, c.tags, c.custom_fields, c.created_at, c.updated_at,
               c.assigned_user_id,
               u.display_name AS assigned_user_name,
        ...
        FROM coexistence.contacts c
        LEFT JOIN coexistence.forgecrm_users u ON u.id = c.assigned_user_id
        WHERE c.wa_number = $1 AND c.contact_number = $2 AND c.organization_id IS NULL
        LIMIT 1
      `, [waNumber, contactNumber]);
...
router.post('/messages/mark-read', async (req, res) => {
  try {
    ...
    // Phase 7: tell other team members' inboxes to clear the unread badge.
    // Tenant scope comes from the request org (assertContactAccess already
    // enforced it) — never from client input beyond the auth'd context.
    if (req.org?.id) {
      try {
        const emitter = require('../realtime/emitter');
        emitter.emitConversationUpdated(req.org.id, {
          waNumber, contactNumber, unreadCount: 0,
        });
        emitter.emitUnreadCountUpdate(req.org.id, {
          waNumber, contactNumber, unreadCount: 0,
        });
      } catch (emitErr) {
        console.error('[messages] realtime emit failed:', emitErr.message);
      }
    }
  } catch (err) {
...
router.get('/contact-names', async (req, res) => {
  try {
    ...
    // Phase 11: org scope.
    const nmOrgId = req.org?.id || null;
    const nmClause = nmOrgId
      ? `AND organization_id = $${params.length + 1}`
      : `AND organization_id IS NULL`;
    if (nmOrgId) params.push(nmOrgId);
    const { rows } = await pool.query(
      `SELECT contact_number, COALESCE(name, profile_name) AS name FROM coexistence.contacts WHERE wa_number = $1 ${assignFilter} ${nmClause}`,
      params
    );
...
router.post('/messages/send', async (req, res) => {
  try {
    ...
    // Phase 7: the sender's own UI renders optimistically; other team members
    // learn about the outbound message via the org room. Tenant scope is the
    // request org (assertContactAccess enforced it).
    if (req.org?.id) {
      try {
        const realtime = require('../realtime/emitter');
        realtime.emitConversationUpdated(req.org.id, {
          waNumber: fromNumber, contactNumber: toNumber,
          lastMessagePreview: trimmedText.slice(0, 280),
        });
        realtime.emitInboundMessage(req.org.id, {
          message_id: localId, wa_number: fromNumber, contact_number: toNumber,
          direction: 'outgoing', message_type: 'text',
          message_body: trimmedText, status: 'sending', timestamp: new Date().toISOString(),
        });
      } catch (emitErr) {
        console.error('[messages] realtime emit failed:', emitErr.message);
      }
