...
adminRouter.get('/mcp/install', adminOnly, (req, res) => {
  ...
  const host = req.headers['x-forwarded-host'] || req.headers.host || (process.env.GREENPILOT_DOMAIN || process.env.FORGECRM_DOMAIN || '');
  const base = `${proto}://${host}`;
  const apiUrl = `${base}/api/mcp/v1`;
  const remoteUrl = `${base}/api/mcp/http/<YOUR_KEY>`;
  const serverPath = process.env.MCP_SERVER_PATH || '/root/GreenPilot/mcp-server/src/index.js';
  res.json({
    ...
    configSnippet: {
      mcpServers: {
        'greenpilot-agents': {
          command: 'node',
          args: [serverPath],
          env: { GREENPILOT_API_URL: apiUrl, GREENPILOT_API_KEY: 'gpk_live_PASTE_YOUR_KEY' },
        },
      },
    },
  });
