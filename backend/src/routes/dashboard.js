...
// Replace /*SCOPE*/ markers with the connected-account + organization +
// per-user clauses.
...
// Organization boundary (Phase 13 hardening): with an org context, aggregates
// cover the org's rows PLUS legacy NULL rows (dual-read, matching every other
// route); without org context (legacy installs) only legacy rows. Secondary
// joins may carry /*ORG:<alias>*/ markers sharing the same org predicate.
function applyScope(sql, params, { admin, uid, kind, wa, orgId }) {
  const out = [...params];
  let clause = '';
  ...
  // Organization boundary — never count another org's rows.
  const primary = kind === 'contacts' ? 'c' : 'ch';
  let orgParam = null;
  if (orgId) {
    out.push(orgId);
    orgParam = `$${out.length}`;
    clause += ` AND (${primary}.organization_id = ${orgParam} OR ${primary}.organization_id IS NULL)`;
  } else {
    clause += ` AND ${primary}.organization_id IS NULL`;
  }
  ...
  if (!admin) {
    ...
    const up = `$${out.length}`;
    const scOrg = orgId
      ? ` AND (sc.organization_id = ${orgParam} OR sc.organization_id IS NULL)`
    ...
    clause += kind === 'contacts'
      ...
      : ` AND EXISTS (SELECT 1 FROM coexistence.contacts sc
                       WHERE sc.wa_number = ch.wa_number
                         ...
                         AND sc.assigned_user_id = ${up}${scOrg})`;
  }

  let result = sql.split('/*SCOPE*/').join(clause);
  result = result.replace(/\/\*ORG:([a-z_]+)\*\//g, (_, alias) => orgId
    ? ` AND (${alias}.organization_id = ${orgParam} OR ${alias}.organization_id IS NULL)`
...
// Scoped to the caller's org (own accounts + legacy unassigned), mirroring
// the whatsapp-accounts list — never another org's number.
async function getConnectedWa(orgId) {
...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const connectedWa = await getConnectedWa(orgId);

    // Helper: run a scoped query (restricted to the connected account + org).
    const q = async (sql, params, kind) => {
      const built = applyScope(sql, params, { admin, uid, kind, wa: connectedWa, orgId });
    ...
    if (admin) {
      // Admin sections are org-scoped (dual-read): the org's rows plus legacy
      // unassigned rows. Cross-org aggregates would leak tenant data.
      const orgPred = (alias) => orgId
        ? `AND (${alias}.organization_id = $1 OR ${alias}.organization_id IS NULL)`
        : `AND ${alias}.organization_id IS NULL`;
      ...
      const [runRow] = (await pool.query(
        `SELECT count(*)::int AS total,
         ...
         WHERE started_at >= NOW() - ($1 * INTERVAL '1 day') ${orgId ? `AND (organization_id = $2 OR organization_id IS NULL)` : `AND organization_id IS NULL`}`,
      ...
      const [bcSummary] = (await pool.query(
        `SELECT count(*)::int AS campaigns
         ...
         WHERE created_at >= NOW() - ($1 * INTERVAL '1 day') ${orgId ? `AND (organization_id = $2 OR organization_id IS NULL)` : `AND organization_id IS NULL`}`,
        orgId ? [days, orgId] : [days]
      )).rows;
      ...
      const [tpl] = (await pool.query(
        `SELECT count(*) FILTER (WHERE status='REJECTED')::int AS rejected,
         ...
         WHERE TRUE ${orgPred('message_templates')}`,
        orgParams
      )).rows;
      ...
      const [waba] = (await pool.query(
        `SELECT count(*) FILTER (WHERE NOT is_active)::int AS inactive FROM coexistence.whatsapp_accounts
          WHERE TRUE ${orgPred('whatsapp_accounts')}`,
        orgParams
      )).rows;
        ...
        if (admin) {
          items = (await pool.query(
            `SELECT name AS primary, ('trigger: ' || trigger_type) AS secondary, status AS meta
               FROM coexistence.chatbots
              WHERE status='active'
                ${orgId ? `AND (organization_id = $1 OR organization_id IS NULL)` : `AND organization_id IS NULL`}
