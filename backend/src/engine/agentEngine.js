...
const { withSecurityPreamble, isSafeHttpUrl } = require('../ai/security');
...
// Tenant-mismatch failures must COMPLETE the queue job (no retry — retrying
// cannot fix a cross-org routing error). The worker catches by code.
function tenantMismatch(jobOrg, actualOrg) {
  const err = new Error(`Tenant mismatch: context org ${jobOrg} != data org ${actualOrg}`);
  err.code = 'tenant-mismatch';
  err.skipRetry = true;
  return err;
}
...
async function buildToolsForAgent(agent, sendCtx = null) {
  ...
  if (mediaGroups.length > 0) {
    ...
    executors['send_media'] = async (args) => {
      ...
      return await sendMediaGroup({ group, waAccountId: sendCtx.waAccountId, contactNumber: sendCtx.contactNumber, organizationId: sendCtx.organizationId });
    };
  }
  ...
  if (agent.crm_tools_enabled && liveContact) {
    ...
    const waNumber = sendCtx.waNumber || await resolveWaNumber(sendCtx.waAccountId, sendCtx.organizationId);
    if (waNumber) {
      const crm = buildCrmTools({ waNumber, contactNumber: sendCtx.contactNumber, organizationId: sendCtx.organizationId });
    ...
    if (waNumber) {
      ...
      executors['escalate_to_human'] = async ({ reason, summary }) => {
        const r = await performHandoff({
          ...
          organizationId: sendCtx.organizationId,
...
async function executeHttpTool(cfg, args) {
  try {
    ...
    // Defense in depth: admin configs are validated at save time, but the
    // final URL (after path-param substitution) is re-checked here so a
    // crafted param value can never redirect the call to internal infra.
    if (!isSafeHttpUrl(url)) {
      return { ok: false, error: 'Configured URL is not allowed.' };
    }
 ...
 */
async function sendMediaGroup({ group, waAccountId, contactNumber, organizationId }) {
  ...
  // The send account must belong to the execution org (Step 13).
  if (organizationId && account.organizationId &&
      String(account.organizationId) !== String(organizationId)) {
    return { sent: false, error: 'WhatsApp account belongs to another organization.' };
  }
    ...
    try {
      const { rows: tRows } = await pool.query(
        `SELECT id, name, language, body, header_type, header_text, footer, buttons,
          ...
          WHERE id = $1
            AND (($2::uuid IS NULL AND organization_id IS NULL) OR organization_id = $2)`,
        [parseInt(group.templateId, 10), organizationId || null],
      );
      const tpl = tRows[0];
 ...
 */
async function buildMessageHistory({ waAccountId, contactNumber, limit, currentInboundText, organizationId }) {
  ...
  const { rows } = waNumber
    ? await pool.query(
        `SELECT direction, message_body, timestamp
          ...
          LIMIT $3`,
        [waNumber, contactNumber, Math.max(1, Math.min(100, limit || 20)), organizationId || null],
      )
    : { rows: [] };
...
async function transcribeInboundIfAudio({ agent, inboundMessageId, agentApiKey, organizationId }) {
  try {
    const { rows } = await pool.query(
      `SELECT message_id, message_type, message_body
        ...
        WHERE message_id = $1
          AND (($2::uuid IS NULL AND organization_id IS NULL) OR organization_id = $2)`,
      [inboundMessageId, organizationId || null],
    );
    const msg = rows[0];
    if (!msg) return '';
    ...
    await pool.query(
      ...
      [text, inboundMessageId, organizationId || null],
    );
    return text;
  } catch (e) {
 ...
 */
async function loadInboundImageBase64({ inboundMessageId, organizationId }) {
  try {
    const { rows } = await pool.query(
...
function withContactContext(systemPrompt, contactNumber) {
  ...
  // Server-side security preamble (non-editable): lead input is untrusted.
  return withSecurityPreamble(withCtx);
}
 ...
 */
async function runAgent({ agentId, contactNumber, inboundMessageId, inboundText, organizationId }) {
  ...
  // Step 3 defense in depth: the engine revalidates tenant context even though
  // the queue worker already did — a job for org A must never execute against
  // org B's agent row or WhatsApp account. Mismatches throw a no-retry error.
  if (organizationId) {
    if (agent.organization_id && String(agent.organization_id) !== String(organizationId)) {
      throw tenantMismatch(organizationId, agent.organization_id);
    }
    const { rows: acctRows } = await pool.query(
      `SELECT organization_id FROM coexistence.whatsapp_accounts WHERE id = $1`,
      [agent.wa_account_id]
    );
    const accountOrg = acctRows[0]?.organization_id || null;
    if (accountOrg && String(accountOrg) !== String(organizationId)) {
      throw tenantMismatch(organizationId, accountOrg);
    }
  }
  ...
  if (!messageText && inboundMessageId && agent.transcribe_audio) {
    ...
    messageText = await transcribeInboundIfAudio({ agent, inboundMessageId, agentApiKey: apiKey, organizationId });
  }
  ...
  if (inboundMessageId && agent.accept_images) {
    inboundImage = await loadInboundImageBase64({ inboundMessageId, organizationId });
  }
  ...
  try {
    const { tools, executors } = await buildToolsForAgent(agent, {
      ...
      contactNumber,
      organizationId: organizationId || agent.organization_id || null,
    });
      ...
      await pool.query(
        `UPDATE coexistence.contacts c
          ...
          WHERE w.id = $1
            ...
            AND (($3::uuid IS NULL AND c.organization_id IS NULL) OR c.organization_id = $3)`,
        [agent.wa_account_id, contactNumber, organizationId || agent.organization_id || null],
    ...
    if (result.finalText) {
      ...
      if (orgId) {
        try {
          ...
          console.error('[agentEngine] sendWhatsAppMessage failed, falling back to enqueueSend:', sendErr.message);
        }
      }
      if (!sentViaService) {
...
async function runCloseSummary({ agentId, waNumber, contactNumber }) {
  ...
  // Phase 9: close-summaries are AI executions too — entitlement + quota gated
  // (daily idempotency key; provider failure charges nothing).
  const { checkQuota, recordAiUsage } = require('../billing/quotas');
  if (agent.organization_id) {
    const quota = await checkQuota(pool, agent.organization_id, 'ai_credits', 1);
    if (!quota.allowed) {
      console.warn(`[closeSummary] agent ${agent.id}: skipped (${quota.reason})`);
    ...
    if (agent.organization_id) {
      ...
      try {
        await recordAiUsage(pool, {
          organizationId: agent.organization_id, userId: null, agentId: agent.id,
          contactNumber, inboundMessageId: `close-summary:${agent.id}:${contactNumber}:${day}`,
          model: agent.llm_model, tokensIn: 0,
          tokensOut: result?.totalOutputTokens || 0, costCredits: 1,
        });
      } catch (usageErr) {
        console.error('[closeSummary] usage ledger failed:', usageErr.message);
      }
    }
