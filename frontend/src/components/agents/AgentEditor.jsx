...
export default function AgentEditor({ agentId, waAccounts, user, navigate, onDone, onCancel }) {
  ...
  const buildPayload = (overrides = {}) => {
    ...
    return {
      ...
      qualifyLeads: form.qualifyLeads,
  ...
  return (
    <div style={{ padding: '24px 24px 80px', width: '100%', boxSizing: 'border-box', fontFamily: FONT }}>
              ...
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer', marginTop: 14 }}>
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', fontSize: 13, fontWeight: 600, color: C.text }}>
                    ...
                    <InfoDot text="When on, the agent can act on the chatting contact inside Green Pilot — save their name, add tags, and set custom fields — so enquiries land in your CRM, not just a sheet. The agent only ever touches the contact it's chatting with." />
                  </div>
                  <div style={{ fontSize: 11, color: C.textMuted, marginTop: 2 }}>
      ...
      <Section title="Lead qualification" subtitle="After the agent handles an inbound message, Green Pilot can qualify the lead: intent, score, and a summary are written to the CRM timeline and announced to the team inbox in realtime. Each qualification consumes 1 AI credit (plus 1 for the agent run itself).">
        <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer' }}>
          <input type="checkbox" checked={form.qualifyLeads}
            onChange={e => setForm(f => ({ ...f, qualifyLeads: e.target.checked }))}
            style={{ width: 16, height: 16, marginTop: 2, cursor: 'pointer' }} />
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: C.text }}>
              Qualify leads after agent replies
            </div>
            <div style={{ fontSize: 11, color: C.textMuted, marginTop: 2 }}>
              Runs only when the subscription is entitled and quota remains; paused chats and disabled AI are skipped automatically.
            </div>
          </div>
        </label>
        {form.qualifyLeads && (
          <div style={{ marginTop: 12 }}>
            <Field label="Qualification rules (optional)" info="Extra guidance for the qualifier, e.g. what counts as qualified for your business.">
              <textarea
                value={form.qualificationRules}
                onChange={e => setForm(f => ({ ...f, qualificationRules: e.target.value }))}
                rows={4}
                placeholder="e.g. A lead is qualified when they share a budget range and a delivery city."
                style={{ ...inputStyle, fontFamily: MONO, fontSize: 13, lineHeight: 1.5, resize: 'vertical', minHeight: 80 }}
              />
            </Field>
          </div>
        )}
