...
const VarPickerButton = ({ onInsert, style }) => {
  ...
  // Custom contact fields from Green Pilot CRM (Admin Settings → Fields), as variable
...
export const getTriggerDisplay = (n) => {
  ...
  // Phase 10 approved triggers (server-enforced; see automation service).
  if (tk === 'message_received') return { title: 'Trigger: Message received', sub: 'Fires on every inbound WhatsApp message' };
  if (tk === 'lead_created') return { title: 'Trigger: New lead', sub: 'First message from a new contact' };
  if (tk === 'lead_qualified') return { title: 'Trigger: Lead qualified', sub: 'When AI qualification completes' };
  if (tk === 'lead_status_changed') return { title: 'Trigger: Lead status changed', sub: 'When a lead qualification status changes' };
  if (tk === 'followup_due') return { title: 'Trigger: Follow-up due', sub: 'When a scheduled follow-up becomes due' };
...
/* ── Action kinds (Phase 10: approved set; unbuilt kinds stay honestly stubbed
   server-side, unsupported types are rejected at save) ── */
const ACTION_KINDS = [
  ...
  { kind:"Add Note",           icon:IC.edit,     valueType:"text",    emptyText:"Note text (supports {{variables}})", placeholder:"Followed up — asked for budget…" },
  { kind:"Schedule Follow-up", icon:IC.clock,    valueType:"text",    emptyText:"When (ISO datetime or 2h / 3d)", placeholder:"2d" },
  { kind:"Update Lead Status", icon:IC.chart,    valueType:"leadStatus", emptyText:"Choose the new status" },
  { kind:"Invoke AI Qualification", icon:IC.ai,   valueType:"none",    emptyText:"Runs lead qualification (1 AI credit)" },
...
export const LEAD_STATUSES = ["qualified", "unqualified", "needs-more-information", "unknown"];
...
const DIRECT_MSG_TYPES = [
  ...
  // dynamic_api removed (Phase 10 SSRF boundary — rejected server-side).
];
...
const CONDITION_SOURCES = [
  ...
  { id:"time",   label:"Time" },
  { id:"qualification", label:"Qualification (AI)" },
];
// Fields the engine resolves from the latest org-scoped qualification.
const QUALIFICATION_FIELDS = ["status", "score", "intent", "summary"];
...
const evalConditionField = (source, field, contact, messageBody = "") => {
  ...
  if (source === "qualification") {
    // Preview-only mirror (no qualification data client-side): unknown.
    return "";
  }
  return "";
};
...
const BLOCK_GROUPS = [
  { title:"Triggers", color:C.brand, items:[
    ...
    { name:"Message Received", type:"trigger", icon:IC.msg,   desc:"Any inbound message",
      defaults:{ triggerKind:"message_received", summary:"Trigger on every inbound WhatsApp message" } },
    { name:"New Lead", type:"trigger", icon:IC.contacts, desc:"First message from a contact",
      defaults:{ triggerKind:"lead_created", summary:"Trigger when a new lead messages first" } },
    { name:"Lead Qualified", type:"trigger", icon:IC.ai, desc:"AI qualification completes",
      defaults:{ triggerKind:"lead_qualified", summary:"Trigger when AI qualification completes" } },
    { name:"Lead Status Changed", type:"trigger", icon:IC.chart, desc:"Qualification status changes",
      defaults:{ triggerKind:"lead_status_changed", summary:"Trigger when a lead status changes" } },
    { name:"Follow-up Due", type:"trigger", icon:IC.clock, desc:"Scheduled follow-up due",
      defaults:{ triggerKind:"followup_due", summary:"Trigger when a scheduled follow-up becomes due" } },
  ]},
  ...
  { title:"Actions", color:"#5B5851", items:[
    ...
    { name:"Assign Lead", type:"action", icon:IC.agent, desc:"Assign to a team member",
      defaults:{ actions:[{ kind:"Assign to BDA", value:"" }], summary:"Assign the lead to a team member" } },
    { name:"Add Note", type:"action", icon:IC.edit, desc:"Timeline note",
      defaults:{ actions:[{ kind:"Add Note", value:"" }], summary:"Add a CRM timeline note" } },
    { name:"Schedule Follow-up", type:"action", icon:IC.clock, desc:"Due reminder",
      defaults:{ actions:[{ kind:"Schedule Follow-up", value:"2d" }], summary:"Schedule a follow-up" } },
    { name:"Update Lead Status", type:"action", icon:IC.chart, desc:"Set qualification status",
      defaults:{ actions:[{ kind:"Update Lead Status", value:"qualified" }], summary:"Update the lead status" } },
    { name:"AI Qualification", type:"action", icon:IC.ai, desc:"Run AI qualification",
      defaults:{ actions:[{ kind:"Invoke AI Qualification", value:"" }], summary:"Run AI lead qualification (1 credit)" } },
  ]},
];
...
const SettingsPanel = ({ node, nodes=[], edges=[], onUpdateNode=()=>{}, onDeleteNode=()=>{}, onDuplicateNode=()=>{}, onSaveAndClose=()=>{}, onToggleDisable=()=>{}, onDeleteButton=()=>{}, onSelectTemplate=()=>{}, onCreateTemplate=()=>{}, templates=[], teamMembers=[], tags=[], contactFields=[], otherAutomations=[], whatsappAccounts=[], assignableUsers=[], sampleContacts=[] }) => {
  ...
  if (node.type === "message") {
      ...
      switch (dt) {
        ...
        case "location":
          return <>
            ...
            <Field label="Location name (optional)"><VarInput value={dd.name || ""} onChange={e=>setDirect({name:e.target.value})} placeholder="e.g. Green Pilot HQ" style={{ padding:"6px 9px", fontSize:11 }}/></Field>
    ...
    const fieldsBySource = { system: WA_SYSTEM_FIELDS, custom: fieldNames, tags: tagNames, bot:["last_intent","bot_state"], time:["Current time","Day of week","Hour of day"], qualification: QUALIFICATION_FIELDS };
    ...
    const TRIGGER_EXPLAIN = {
      ...
      qr: {
        ...
        source: "wa", sourceLabel: "WhatsApp inbound message webhook",
        title: "QR code scan",
        body: "Technically identical to a click-to-chat link — the QR encodes a wa.me URL with a pre-filled message that identifies the scan source. WhatsApp doesn't know it came from a QR; Green Pilot attributes it via the pre-filled text."
      },
      newContact: {
      ...
      message_received: {
        ...
        source: "wa", sourceLabel: "WhatsApp inbound message webhook",
        title: "Any inbound message",
        body: "Fires on every inbound WhatsApp message from any contact. Use with conditions to narrow it down."
      },
      lead_created: {
        icon: IC.contacts, color: C.purpleDark, bg: C.purpleBg, border: "#C7C2F4",
        source: "wa", sourceLabel: "WhatsApp inbound message webhook (first-time contact)",
        title: "New lead",
        body: "Fires the first time an unknown contact messages. Use for welcome flows and lead capture."
      },
      lead_qualified: {
        icon: IC.ai, color: C.brand, bg: C.brandBg, border: C.brandBright,
        source: "bsp", sourceLabel: "Workspace event · AI qualification completed",
        title: "Lead qualified",
        body: "Fires when AI qualification completes for a conversation. Qualify first with an agent that has lead qualification enabled."
      },
      lead_status_changed: {
        icon: IC.chart, color: C.brand, bg: C.brandBg, border: C.brandBright,
        source: "bsp", sourceLabel: "Workspace event · qualification status changed",
        title: "Lead status changed",
        body: "Fires when a lead's qualification status changes — by AI or by an Update Lead Status action."
      },
      followup_due: {
        icon: IC.clock, color: C.orange, bg: "#FFF4E5", border: "#F5C97B",
        source: "bsp", sourceLabel: "Workspace event · scheduled follow-up due",
        title: "Follow-up due",
        body: "Fires when a scheduled follow-up becomes due. Pair with a Send Message action for reminders."
      },
    };
    ...
    content = (<>
      ...
      <Field label="Trigger type" hint="What starts this automation. New kinds are server-enforced.">
        <Select
          value={tk}
          onChange={(e)=>{
            const next = e.target.value;
            const disp = getTriggerDisplay({ ...node, triggerKind: next });
            onUpdateNode(node.id, { triggerKind: next, title: disp.title, sub: disp.sub });
          }}
    ...
    const aiContext = node.aiContext !== undefined ? node.aiContext : "We are Green Pilot Realty, a Chennai-based real-estate agency. Our active inventory: 2BHK & 3BHK apartments in Anna Nagar (₹85L–₹1.2Cr), Adyar (₹1.4Cr–₹2.1Cr), and Velachery (₹70L–₹95L). Site visits Mon–Sat 11 AM – 6 PM. Token amount ₹50,000 (refundable).";
                  ...
                  {assignMode === 'variable' ? (
                    <>
                      ...
                      <div style={{ fontSize:10, color:C.text5, fontWeight:500, lineHeight:1.4 }}>
                        The resolved value must be a numeric user ID (team member ID). If it's missing, disabled, or not an admin/BDA Sales user, the step fails.
                      </div>
                    </>
                  ) : (
                    <>
...
const BuilderToolbar = ({ automationName, status, onBack, onSave, isDirty, saving, onToggleStatus, toggleBusy, onPreview, showPreview, onServerTest, serverTestBusy, activeTab, onTabChange, onUndo, onRedo, canUndo, canRedo, onZoomIn, onZoomOut, onFit, onAutoLayout, onExport }) => {
  ...
  return (
    <div style={{ background:"#fff", borderBottom:`1px solid ${C.cardBorder}`, padding:"9px 16px", display:"flex", alignItems:"center", gap:10, flexShrink:0 }}>
      ...
      <Btn kind="ghost" icon={IC.play(12)} onClick={onServerTest} title="Run this flow on the server with simulated side effects (no customer messages, no quota)"> {serverTestBusy ? "Testing…" : "Server test"}</Btn>
      {onExport && (
            ...
            <div style={{ flex: 1, minHeight: 0, position: "relative", borderRadius: 46.5, overflow: "hidden", display: "flex", flexDirection: "column", background: "#075E54" }}>
              ...
              <div style={{ background:"#075E54", paddingTop:50, paddingBottom:8, paddingLeft:12, paddingRight:12, color:"#fff", fontFamily:"-apple-system, 'SF Pro Display', system-ui, sans-serif", flexShrink:0, position:"relative", zIndex:1 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  ...
                  <div style={{ width:30, height:30, borderRadius:"50%", background:`linear-gradient(135deg,${C.brandBright},${C.brand})`, display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontSize:12, fontWeight:700, flexShrink:0 }}>G</div>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontSize:13, fontWeight:600, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>Green Pilot Automation</div>
...
const AutomationBuilderView = ({ automation, onBack, onSave, onToggleStatus, activeTab, onTabChange, initialExecutionId, onNavigate }) => {
  ...
  // Phase 10 server test run (real engine walk, simulated side effects).
  const [serverTest, setServerTest] = useState(null); // null | {running:true} | {result} | {error}
  const runServerTest = async () => {
    if (!automation?.id) { setServerTest({ error: "Save the automation first, then run a server test." }); return; }
    if (isDirty) { setServerTest({ error: "You have unsaved changes — save first so the test runs the current flow." }); return; }
    setServerTest({ running: true });
    try {
      const res = await api.automations.testRun(automation.id, { messageText: "Server test run" });
      setServerTest({ result: res });
    } catch (e) {
      setServerTest({ error: e.message || "Server test failed" });
    }
  };
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%", background:C.pageBg, fontFamily:"'DM Sans', system-ui, sans-serif" }}>
      <BuilderToolbar
        ...
        serverTestBusy={!!(serverTest && serverTest.running)}
      ...
      {serverTest && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.35)", zIndex:80, display:"flex", alignItems:"center", justifyContent:"center" }} onClick={()=>setServerTest(null)}>
          <div style={{ background:"#fff", borderRadius:12, padding:"24px", width:520, maxHeight:"80vh", overflowY:"auto", boxShadow:"0 20px 50px rgba(0,0,0,.2)" }} onClick={e=>e.stopPropagation()}>
            <div style={{ fontSize:16, fontWeight:700, color:C.text1, marginBottom:4 }}>Server test run</div>
            <div style={{ fontSize:12, color:C.muted, marginBottom:14 }}>Real engine walk on the saved flow. Side effects are simulated — no customer messages, no quota spent.</div>
            {serverTest.running && <div style={{ fontSize:13, color:C.text3 }}>Running…</div>}
            {serverTest.error && <div style={{ fontSize:13, color:C.redDark, fontWeight:600 }}>{serverTest.error}</div>}
            {serverTest.result && (
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                <div style={{ fontSize:12, color:C.text2 }}>
                  Execution #{serverTest.result.execution?.id} · status <strong>{serverTest.result.execution?.status}</strong>
                </div>
                {(serverTest.result.steps || []).map(s => (
                  <div key={s.id} style={{ border:`1px solid ${C.cardBorder}`, borderRadius:8, padding:"8px 10px", fontSize:12 }}>
                    <div style={{ fontWeight:700, color:C.text1 }}>{s.node_type} · {s.status}</div>
                    {s.output_data && <pre style={{ margin:"6px 0 0", fontSize:11, whiteSpace:"pre-wrap", wordBreak:"break-word", color:C.text3 }}>{JSON.stringify(s.output_data).slice(0, 600)}</pre>}
                    {s.error_message && <div style={{ color:C.redDark, marginTop:4 }}>{s.error_message}</div>}
                  </div>
                ))}
                {(serverTest.result.steps || []).length === 0 && (
                  <div style={{ fontSize:12, color:C.muted }}>No steps recorded.</div>
                )}
              </div>
            )}
            <div style={{ display:"flex", gap:8, justifyContent:"flex-end", marginTop:16 }}>
              <Btn kind="primary" onClick={()=>setServerTest(null)}>Close</Btn>
            </div>
          </div>
        </div>
      )}
