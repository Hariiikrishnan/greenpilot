...
export default function ContactList({ waNumber, width = 380, selectedContact, onSelectContact, refreshKey, user }) {
  ...
  // Polling refresh (every 30s) stays as the fallback; realtime below makes
  // the inbox update instantly on inbound traffic.
  const { data, loading, refetch } = usePolling(() => api.contacts(waNumber, '30d'), 30000);

  // Phase 7 canonical realtime: any org-scoped inbox event refreshes the
  // list (debounced + coalesced so bulk inbound doesn't storm the API).
  // The socket layer already filtered to this org; waNumber matching keeps
  // multi-number inboxes from refetching on unrelated traffic.
  const listTimer = useRef(null);
  useEffect(() => () => { if (listTimer.current) clearTimeout(listTimer.current); }, []);
  const onInboxEvent = useCallback((ev) => {
    if (ev.type !== 'inbound-message' && ev.type !== 'conversation-updated' && ev.type !== 'unread-count-update') return;
    const d = ev.data || {};
    if (d.waNumber && String(d.waNumber).replace(/\D/g, '') !== String(waNumber).replace(/\D/g, '')) return;
