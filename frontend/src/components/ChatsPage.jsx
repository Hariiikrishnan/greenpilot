...
// Green Pilot storage keys (migrated from the legacy `forgecrm.*` namespace;
// existing prefs are carried over on first read, then saved under the new key).
const LS_WIDTH = 'greenpilot.chats.contactWidth';
const LS_COLLAPSED = 'greenpilot.chats.navCollapsed';
const LEGACY_KEYS = {
  [LS_WIDTH]: 'forgecrm.chats.contactWidth',
  [LS_COLLAPSED]: 'forgecrm.chats.navCollapsed',
};
...
export default function ChatsPage({ subParts = [], navigate, user }) {
  ...
  const [navCollapsed, setNavCollapsed] = useState(
    () => readPref(LS_COLLAPSED) === '1'
  );
  const [contactWidth, setContactWidth] = useState(() => {
    const v = parseInt(readPref(LS_WIDTH), 10);
