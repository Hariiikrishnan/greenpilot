...
export default function ContactsPage({ user, onNavigate }) {
  ...
  // Phase 11: bumped by org-scoped CRM realtime events so the open lead
  // detail refreshes live (LeadDetail re-fetches on change).
  const [leadRefreshKey, setLeadRefreshKey] = useState(0);
  const onCrmEvent = useCallback((ev) => {
    if ([
      'lead-updated', 'lead-assigned', 'lead-status-changed',
      'followup-created', 'followup-completed', 'activity-created',
      'inbound-message', 'lead-qualified',
    ].includes(ev.type)) {
      setLeadRefreshKey(k => k + 1);
    }
  }, []);
  useRealtime(onCrmEvent);
  ...
  // Ctrl+V paste a sheet into the open modal (Green Pilot convention: file inputs support paste)
  useEffect(() => {
      ...
      {detailContact && (
        <div style={{
          ...
          <div style={{
            ...
            {/* Phase 11 lead workspace (view mode): qualification, stage,
                assignment, timeline, notes, calls, follow-ups. */}
            {detailMode === 'view' && detailContact && (
              <LeadDetail
                waNumber={selectedNumber}
                contactNumber={detailContact.contact_number}
