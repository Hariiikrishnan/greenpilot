...
export default function PipelinesPage({ user }) {
  ...
  const loadPipelines = useCallback(async (keepId) => {
    setLoading(true);
    try {
      // Phase 12 CRM defaults: idempotent bootstrap — a new organization with
      // no pipelines yet gets the standard Sales Pipeline on first visit.
      // initDefault is a no-op ({ created: false }) when pipelines exist.
      try { await api.pipelines.initDefault(); } catch { /* non-admins or pre-migration: list below decides */ }
  ...
  // Phase 11 live board: org-scoped CRM events (server already filtered to
  // this org) refresh deals + metrics without polling.
  const selectedRef = useRef(selectedId);
  selectedRef.current = selectedId;
  const onCrmEvent = useCallback((ev) => {
    if (['lead-updated', 'lead-assigned', 'lead-status-changed'].includes(ev.type)) {
