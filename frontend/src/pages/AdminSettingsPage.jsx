...
import {
  ...
  PlugZap, Terminal, ChevronRight, CreditCard, Building2, User, RefreshCw,
} from 'lucide-react';
...
import OrganizationTab from '../components/settings/OrganizationTab.jsx';
...
import MetaWhatsAppConnect from '../components/MetaWhatsAppConnect.jsx';
...
function GeneralTab({ onLogout, user }) {
  // Phase 12: timezone is real organization state (GET/PUT /v1/settings/organization),
  // never local-only. Owners/admins may change it; members see it read-only.
  ...
  useEffect(() => {
    ...
    api.settings.getOrganization()
      .then(o => {
        ...
        setCanEdit(o?.role === 'owner' || o?.role === 'admin');
      })
  ...
  const handleTimezone = async (tz) => {
    setTimezone(tz);
    if (!canEdit) return; // read-only for members — no mock persistence
    setSaving(true);
    setError('');
    setSaved(false);
    try {
      await api.settings.updateOrganization({ timezone: tz });
      setSaved(true);
    } catch (err) {
      ...
      <div style={{ maxWidth: '100%' }}>
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: C.textSecondary, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12 }}>
            Timezone {saving ? '(saving…)' : saved ? '(saved)' : ''}
          </div>
          {loading ? (
            <div style={{ fontSize: 13, color: C.textMuted }}>Loading workspace timezone…</div>
          ) : (
            <SearchableSelect
              value={timezone}
          ...
          {error && (
            <div style={{ marginTop: 8, fontSize: 12, color: '#A32D2D' }}>{error}</div>
          )}
          {!canEdit && !loading && (
            <div style={{ marginTop: 8, fontSize: 12, color: C.textMuted }}>
              Read-only — only owners and admins can change the workspace timezone.
            </div>
          )}
        </div>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {/* Account deletion is intentionally unavailable in-app: there is no
                deletion API, and the previous control only showed a fake
                success alert. Owners manage membership under Organizations;
                account removal is a support-assisted operation until the
                members API ships a delete path. */}
            <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 16, marginTop: 6 }}>
              <div style={{ fontSize: 12, color: C.textSecondary, maxWidth: 460 }}>
                Need to close this workspace? Contact support — in-app account
...
function WhatsappAccountsTab() {
  ...
  const handleVerify = async (acc) => {
    ...
    try {
      const res = await api.whatsappAccounts.verify(acc.id);
      if (res?.health?.healthy) {
        alert('✓ WhatsApp connection verified! Healthy and active with Meta Cloud API.');
      } else {
        alert(`⚠ WhatsApp needs attention: ${res?.health?.lastError || 'Could not verify connection'}`);
      }
      await refresh();
    } catch (err) {
  ...
  const handleDisconnect = async () => {
    ...
    try {
      await api.whatsappAccounts.disconnect(disconnectModal.account.id);
  ...
  const hasActiveAccount = accounts.some(a => a.isActive && a.connectionStatus !== 'disconnected');
    ...
    <div style={{ flex: 1, padding: 24, overflow: 'auto', fontFamily: FONT }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: C.textMuted, margin: '4px 0 0' }}>
            Connect your WhatsApp Business Account (WABA) to send templates, broadcasts and AI-driven messages.
          </p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <MetaWhatsAppConnect onConnected={refresh} />
          <button onClick={startCreate} style={{
            display: 'flex', alignItems: 'center', gap: 6,
            padding: '8px 12px', background: 'transparent', color: C.textSecondary,
            border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 12,
            fontWeight: 600, cursor: 'pointer', fontFamily: FONT,
          }}>
            <Plus size={14} /> Manual credentials
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{
          ...
          <MessageSquare size={40} style={{ opacity: 0.7, color: '#25D366', marginBottom: 14 }} />
          ...
          <p style={{ fontSize: 13, color: C.textSecondary, margin: '0 0 20px', lineHeight: 1.5 }}>
            Connect your business WhatsApp account directly through Meta. No need to manually copy IDs or permanent system user tokens.
          </p>
        ...
        <div style={{ background: C.cardBg, border: `1px solid ${C.border}`, borderRadius: 10, overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {accounts.map(acc => {
                const isConn = acc.isActive && acc.connectionStatus !== 'disconnected';
                return (
                  <tr key={acc.id} style={{ borderBottom: `1px solid ${C.border}` }}>
                    <td style={tdStyle}>
                      <span style={{ fontWeight: 600 }}>{acc.displayName || 'WhatsApp Account'}</span>
                    </td>
                    ...
                    <td style={tdStyle}>
                      {(() => {
                        const status = (acc.connectionStatus || (acc.isActive ? 'CONNECTED' : 'DISCONNECTED')).toUpperCase();
                        const styles = {
                          CONNECTED: { bg: '#E1F5EE', fg: '#0F6E56', label: 'Connected' },
                          CONNECTING: { bg: '#E3F2FD', fg: '#1565C0', label: 'Connecting…' },
                          ERROR: { bg: '#FEEFC3', fg: '#B06000', label: 'Needs attention' },
                          PENDING: { bg: '#FFF8E1', fg: '#F57F17', label: 'Pending' },
                          DISCONNECTED: { bg: '#EEEDE8', fg: C.textMuted, label: 'Disconnected' },
                        };
                        const s = styles[status] || styles.DISCONNECTED;
                        return (
                          <span
                            title={acc.lastErrorMessage || (status === 'CONNECTED' ? (acc.webhookSubscribed ? 'Connected & Webhook Active' : 'Connected') : '')}
                            style={{
                    ...
                    <td style={{ ...tdStyle, textAlign: 'right' }}>
                      <button
                        ...
                        style={iconBtnStyle}
                        title="Verify connection with Meta"
                        disabled={verifyingId === acc.id}
                      >
                        <RefreshCw size={14} className={verifyingId === acc.id ? 'spin' : ''} />
                      </button>
                      <button onClick={() => startEdit(acc)} style={iconBtnStyle} title="View / edit credentials">
                        <Eye size={14} />
                      </button>
                      {acc.isActive && acc.connectionStatus !== 'DISCONNECTED' && (
                        <button
      ...
      {disconnectModal.open && disconnectModal.account && (
        <div onClick={() => setDisconnectModal({ open: false, account: null })} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--c-cardBg)', borderRadius: 14, width: 440, boxShadow: C.shadowLg, padding: 24, fontFamily: FONT }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#FCEBEB', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#A32D2D' }}>
                <LogOut size={18} />
              </div>
              <h3 style={{ fontSize: 16, fontWeight: 700, color: C.text, margin: 0 }}>Disconnect WhatsApp Account</h3>
            </div>
            <p style={{ fontSize: 13, color: C.textSecondary, lineHeight: 1.5, margin: '0 0 16px' }}>
              Disconnecting <strong>{disconnectModal.account.displayName || disconnectModal.account.displayPhoneNumber || 'this WhatsApp account'}</strong> will stop message sending and receiving through this number.
            </p>
            <div style={{ padding: '10px 14px', background: 'var(--c-hover)', border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 12, color: C.textMuted, marginBottom: 20 }}>
              ✓ All contacts, leads, conversations, and campaign metrics are completely preserved.
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
              <button
                onClick={() => setDisconnectModal({ open: false, account: null })}
                disabled={disconnecting}
                style={{ padding: '8px 16px', background: 'var(--c-cardBg)', color: C.text, border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: FONT }}
              >
                Cancel
              </button>
              <button
                ...
                disabled={disconnecting}
                style={{ padding: '8px 16px', background: '#dc2626', color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: disconnecting ? 'wait' : 'pointer', fontFamily: FONT, display: 'flex', alignItems: 'center', gap: 6 }}
              >
                {disconnecting && <Loader2 size={14} className="spin" />}
                Disconnect
              </button>
            </div>
          </div>
        ...
        {showInstall && (
          <div style={{ marginTop: 16 }}>
            ...
            <ol style={{ fontSize: 13, color: C.text, lineHeight: 1.8, paddingLeft: 18, margin: '0 0 10px' }}>
              ...
              <li>Say <i>“create a Green Pilot agent”</i> — it asks the setup questions.</li>
            </ol>
            {install?.remoteUrl && (
            ...
            <ol style={{ fontSize: 13, color: C.text, lineHeight: 1.8, paddingLeft: 18, margin: '0 0 16px' }}>
              <li>On the machine running Claude Desktop, install the server: <code style={{ fontFamily: MONO, fontSize: 12 }}>cd {install?.serverPath?.replace('/src/index.js', '') || '/root/greenpilot/mcp-server'} && npm install</code></li>
              <li>Open Claude Desktop → <b>Settings → Developer → Edit Config</b> and add the block below (paste your key).</li>
              <li>Fully quit and reopen Claude Desktop. The <code style={{ fontFamily: MONO, fontSize: 12 }}>greenpilot-agents</code> tools appear.</li>
            </ol>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300, fontFamily: FONT }}>
          <div style={{ background: C.cardBg, borderRadius: 14, padding: 24, width: 480, boxShadow: C.shadowLg }}>
            ...
            <div style={{ fontSize: 13, color: C.textSecondary, marginBottom: 12, lineHeight: 1.6 }}>
              Copy this key now — it won’t be shown again. Store it in your Claude Desktop config as <code style={{ fontFamily: MONO }}>GREENPILOT_API_KEY</code>.
            </div>
...
export default function AdminSettingsPage({ onLogout, onNavigate, subParts = [], navigate, user }) {
  ...
  const VALID_TABS = ['general', 'organization', 'team', 'profile', 'tags', 'category', 'fields', 'whatsapp-accounts', 'billing', 'integrations', 'google-integrations', 'mcp', 'users'];
  // Admins see every tab; other roles see only tabs granted by their pages
  // (e.g. Sales users get General only). Owners/admins of the active org
  // additionally get the workspace-setup tabs (organization, team, profile,
  // whatsapp-accounts, billing) so a SaaS customer can self-serve setup.
  // Instance-admin surfaces (users, mcp, integrations) stay pages-gated.
  const orgRole = useOrgRole();
  const MANAGER_TABS = ['general', 'organization', 'team', 'profile', 'whatsapp-accounts', 'billing'];
  const visibleTabs = (user?.role === 'admin' || !Array.isArray(user?.pages))
    ? TABS
    : TABS.filter(t => user.pages.includes(`admin-settings:${t.key}`)
      || ((orgRole?.isManager) && MANAGER_TABS.includes(t.key)));
