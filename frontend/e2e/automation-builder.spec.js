...
const LOGIN_EMAIL = process.env.LOGIN_EMAIL || 'admin@greenpilot.io';
...
async function login(page) {
  ...
  await page.waitForSelector('text=GREEN PILOT', { timeout: 15000 });
}
...
test.describe('Automation Builder — Node Settings', () => {
  ...
  test('message node template selection', async ({ page }) => {
    ...
    await expect(page.getByText('welcome to Green Pilot').first()).toBeVisible();
  });
