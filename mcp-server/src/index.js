#!/usr/bin/env node
// Green Pilot Agent Builder — MCP server.
//
// A thin, well-described client over the Green Pilot MCP API
...
// BEFORE calling create_agent — see the `create-greenpilot-agent` prompt and the
// tool descriptions.
...
const API_URL = (process.env.GREENPILOT_API_URL || process.env.FORGECHAT_API_URL || '').replace(/\/$/, '');
const API_KEY = process.env.GREENPILOT_API_KEY || process.env.FORGECHAT_API_KEY || '';

if (!API_URL || !API_KEY) {
  console.error('[greenpilot-mcp] GREENPILOT_API_URL and GREENPILOT_API_KEY env vars are required.');
  process.exit(1);
}
...
const server = new McpServer({ name: 'greenpilot-agents', version: '1.0.0' });
...
const GUIDE = `You are creating a Green Pilot WhatsApp AI agent. Gather and CONFIRM the full configuration with the user before calling create_agent. Offer real options fetched from their account — never guess ids, spreadsheets, tabs, models, or numbers.
...
server.registerPrompt('create-greenpilot-agent', {
  title: 'Create a Green Pilot agent',
  description: 'Guided flow to create and configure a Green Pilot WhatsApp AI agent (asks the right questions, then creates it).',
  argsSchema: {},
}, () => ({
...
server.registerPrompt('create-forgechat-agent', {
  title: 'Create a Green Pilot agent (legacy)',
  description: 'Guided flow to create and configure a Green Pilot WhatsApp AI agent.',
  argsSchema: {},
}, () => ({
  messages: [{ role: 'user', content: { type: 'text', text: GUIDE } }],
}));
...
console.error('[greenpilot-mcp] ready');
